/**
 * SDOX Renderer — TypeScript port
 * Converts an SDOX AST into HTML string for the preview panel.
 * Based on the Chrome extension renderer.
 */

import { SdoxNode } from './sdoxParser';
...
function renderChildren(children: SdoxNode[]): string {
...
function renderCodeBlock(code: string, language: string, title?: string, _showLineNumbers?: boolean): string {
    ...
    <div class="sdox-code-wrap">
        ${title ? `<div class="code-header"><span class="code-title">${escapeHtml(title)}</span><span class="code-lang">${escapeHtml(language)}</span></div>` : (language && language !== 'text' ? `<div class="code-header"><span class="code-title"></span><span class="code-lang">${escapeHtml(language)}</span></div>` : '')}
        <div class="code-body">
 ...
 */
export function renderSdoxAST(node: SdoxNode): string {
    if (!node) { return ''; }
    ...
    switch (node.type) {
        ...
        case 'note': {
            const noteType = String(attr.type || 'info');
            return `
        ...
        case 'image':
            ...
            <figure class="sdox-figure">
                <div class="img-frame"><img src="${escapeHtml(attr.src)}" alt="${escapeHtml(attr.alt || '')}" ${attr.width ? `width="${escapeHtml(attr.width)}"` : ''} ${attr.height ? `height="${escapeHtml(attr.height)}"` : ''} /></div>
        ...
        case 'code':
            return renderCodeBlock(
                content,
                String(attr.language || 'text'),
                attr.title ? String(attr.title) : undefined,
        ...
        case 'instruction':
            ...
            <div class="sdox-ai-block ai-instruction">
                <div class="ai-header"><span class="ai-icon">📝</span><span class="ai-label">Instruction</span>
                    ${attr.priority ? `<span class="model-badge" style="background:${priorityColors[String(attr.priority)] || '#64748b'}33;color:${priorityColors[String(attr.priority)] || '#64748b'}; border-color: ${priorityColors[String(attr.priority)] || '#64748b'}80;">${escapeHtml(attr.priority)}</span>` : ''}
                </div>
                <div class="ai-body">${node.children.length > 0 ? childrenHTML : `<p>${escapeHtml(content)}</p>`}</div>
            </div>`;
...
/**
 * Render a full document AST to a complete HTML string.
 */
export function renderDocument(ast: SdoxNode): string {
    return renderSdoxAST(ast);
}
